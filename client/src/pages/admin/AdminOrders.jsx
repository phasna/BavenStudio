...
export default function AdminOrders() {
  ...
  return (
    <div>
      {orders.length === 0 ? (
        <p style={{ color: 'var(--color-neutral-grey)' }}>{t('admin.orders.empty')}</p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {orders.map((order) => (
            <AdminOrderRow key={order.id} order={order} t={t} language={language} />
          ))}
        </div>
      )}
    </div>
...
function AdminOrderRow({ order, t, language }) {
      ...
      >
        <div>
          ...
          <div className="eyebrow" style={{ color: 'var(--color-neutral-grey)' }}>
            {date} · {order.fullName} · {order.email}
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <span className="eyebrow" style={{ color: 'var(--color-neutral-grey)' }}>
            {itemCount} {itemCount > 1 ? t('account.orderItemsPlural') : t('account.orderItems')}
          </span>
          <span
            className="eyebrow"
            style={{
          ...
          <span style={{ transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s ease' }}>▾</span>
        </div>
      </button>
